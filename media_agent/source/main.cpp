...
int main() {
  ...
  auto rest_server = std::make_shared<HttpFacade>();
  rest_server->init();
  rest_server->start();
  return x;
...
//
//  auto slot = reader->sig_new_packet_.connect(
//      [t = decoder.get()](auto &&PH1, auto &&PH2) { t->slot_new_packet(std::forward<decltype(PH1)>(PH1), std::forward<decltype(PH2)>(PH2)); });
//
//  auto slot0 = decoder->signal_new_frame.connect(
//      [t = encoder.get()](auto &&PH1, auto &&PH2) { t->slot_new_frame(std::forward<decltype(PH1)>(PH1), std::forward<decltype(PH2)>(PH2)); });
//
//  auto slot1 = encoder->signal_new_packet.connect(
//      [t = writer.get()](auto &&PH1, auto &&PH2) { t->slot_new_packet(std::forward<decltype(PH1)>(PH1), std::forward<decltype(PH2)>(PH2)); });
//
